...
def make_experiment(experiment_name, env_id, args):
    base_args = [
...
def main():
    ...
    for e in envs:
        experiments.append([make_experiment(experiment_name, e, args) for args in arg_list])

    # Unpack the list of lists into a single list of experiments to run
    import itertools
    experiments = list(itertools.chain(*experiments))

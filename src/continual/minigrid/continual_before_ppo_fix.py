...
if __name__ == "__main__":
    ...
    for indx, cur_env in enumerate(sequence_keys):
        ...
        for iteration in range(1, args.num_iterations + 1):
            ...
            # CLEAR Logic
            # After the above, pass all 8 envs into the CLEAR buffer and merge 2 old envs with the new
            ...
            if args.clear:
                if indx == 0: # Fill buffer first
                    clear.update_buffer(rollout)
                    is_replay = torch.zeros((args.num_steps, args.num_envs), device=device)
                else:
                    rollout, is_replay = clear.blend_rollout(
                        rollout, int(args.clear_ratio * args.num_envs)
                    )
            else:
                # No CLEAR, just mark everything on-policy
                is_replay = torch.zeros((args.num_steps, args.num_envs), device=device)
            ...
            # Column mask is constant across time after blend
            replay_cols = is_replay[0].bool()
            ...
            # GAE for all columns by default (will override replay envs next)
            ...
            for t in reversed(range(T)):
                if t == T - 1:
                    next_non_terminal = 1.0 - next_done
                    next_values = bootstrap_values # will be overwritten later but use instead of next_value
                else:
                    next_non_terminal = 1.0 - D[t + 1]
                    next_values = Vb[t + 1]
                delta = R[t] + args.gamma * next_values * next_non_terminal - Vb[t]
            ...
            # If CLEAR is enabled and there are old (replay) experiences in the rollout
            if args.clear and replay_cols.any():
                replay_idx = replay_cols.nonzero(as_tuple=True)[0] # replay envs indices
                # Extract replay variables
                O_rp, A_rp = O[:, replay_idx], A[:, replay_idx].long()
                Lb_rp, Vb_rp = Lb[:, replay_idx], Vb[:, replay_idx]
                D_rp, R_rp = D[:, replay_idx], R[:, replay_idx]
                boot_rp = boot_b[replay_idx]

                c_cfc_state = rollout['states']['cfc'][replay_idx] if rollout['states']['cfc'] is not None else None
                c_lstm_states = (
                    rollout['states']['lstm'][0][:, replay_idx], # Take only the (h, c) of the replay envs
                    rollout['states']['lstm'][1][:, replay_idx]
                ) if args.use_lstm else (None, None)

                # Calculate advanatages for replay experiences
                # Same as previous calculation, only including v-trace parameters
                with torch.no_grad():
                    # Get the current policies logits and values for calculations
                    logp_rp, Vcur_rp = agent.evaluate_actions(O_rp, A_rp, cfc_states=c_cfc_state, lstm_states=c_lstm_states, dones=D_rp)
                    rho = (logp_rp - Lb_rp).exp().clamp(max=vtrace_clip_rho)
                    c = (logp_rp - Lb_rp).exp().clamp(max=vtrace_clip_c)

                    vs = torch.empty_like(Vb_rp)
                    next_v = boot_rp
                    for t in reversed(range(T)):
                        td = R_rp[t] + args.gamma * next_v * (1.0 - D_rp[t]) - Vb_rp[t]
                        if t == T - 1:
                            vs[t] = Vb_rp[t] + rho[t] * td
                        else:
                            vs[t] = Vb_rp[t] + rho[t] * td + args.gamma * c[t] * (vs[t+1] - Vb_rp[t+1])
                        next_v = vs[t]

                # Overwrite only the replay columns; keep rest of rollout with on-policy behaviour
                advantages[:, replay_idx] = (rho * (vs - Vcur_rp)).detach()
                returns[:, replay_idx] = vs.detach()
